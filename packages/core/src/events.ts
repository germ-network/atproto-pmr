...
/** Unconditional "you are live now" — sent whether or not `#pool` was. */
export function encodeDrainedFrame(): Uint8Array {
    return encodeFrame("drained", new Map())
}
...
/**
 * Reconnect-drain: every pending message across every mailbox, oldest
 * page first, then the pool notice (only if there is one), then `#drained`
 * — always last, always sent, so the client has an unconditional signal
 * that backlog is over even when the pool is empty.
 *
